#!/bin/bash
# Final polling for spider, imp, wraith — download walk frames + retry imp anim queue if slot was full.
set -uo pipefail
...
queue_imp_anim() {
  curl -sS --max-time 30 -X POST \
    ...
    -d '{"character_id":"16d9e44d-d445-4c11-99df-573e644d765d","template_animation_id":"walking-4-frames","mode":"template","directions":["south","north","east","west"]}' \
...
while true; do
  ...
  # Retry imp anim queue if not yet successful
  if [ "$IMP_ANIM_QUEUED" = "0" ]; then
    res=$(queue_imp_anim)
    if [[ "$res" == *"200"* ]] || [[ "$res" == *"201"* ]]; then
      echo "[$(date +%H:%M:%S)] imp anim queued"
      IMP_ANIM_QUEUED="1"
    else
  ...
  for i in 0 1 2; do
    ...
    slug="${entry%%:*}"
    cid="${entry##*:}"
    n=$(walk_dirs_done "$cid")
    if [ "$n" -ge "4" ]; then
      saved=$(dl_walk "$slug" "$cid")
      echo "[$(date +%H:%M:%S)] $slug walk frames: $saved"
      DONE[$i]="1"
    else
      remaining=$((remaining+1))
    fi
